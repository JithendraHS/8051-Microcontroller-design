...
;------------------------------------------------------------
;	buffer.c:33: __xdata int16_t get_buffer_size(uint32_t min, uint32_t max, uint32_t divisible) {
;	-----------------------------------------
...
_get_buffer_size:
	...
	inc	dptr
	movx	@dptr,a
;	buffer.c:34: __xdata int16_t buffer_size = 0;
...
00117$:
;	buffer.c:38: __xdata uint8_t char_received = echo(); // Read a character from UART
	push	ar3
	push	ar2
	push	ar1
	push	ar0
...
00147$:
00112$:
;	buffer.c:44: if ((buffer_size < min) || (buffer_size > max)) {
	mov	dptr,#_get_buffer_size_buffer_size_65536_20
	movx	a,@dptr
...
;	buffer.c:46: "value\r\n");
	...
	pop	ar2
	pop	ar3
;	buffer.c:47: buffer_size = 0;
	mov	dptr,#_get_buffer_size_buffer_size_65536_20
	clr	a
	movx	@dptr,a
	inc	dptr
	movx	@dptr,a
;	buffer.c:48: continue; // Input is invalid, continue reading user input
	ljmp	00117$
00105$:
;	buffer.c:49: } else if (buffer_size % divisible) {
...
;	buffer.c:51: "value\r\n", divisible);
	...
	pop	ar2
	pop	ar3
;	buffer.c:52: buffer_size = 0;
	mov	dptr,#_get_buffer_size_buffer_size_65536_20
	clr	a
	movx	@dptr,a
	inc	dptr
	movx	@dptr,a
;	buffer.c:53: continue; // Input is not divisible, continue reading user input
	ljmp	00117$
;	buffer.c:55: break; // Input is valid, exit the loop
00113$:
;	buffer.c:57: } else if ((char_received >= '0') && (char_received <= '9')) {
...
00151$:
	...
	add	a,#0xff - 0x39
	jc	00109$
;	buffer.c:58: char_received = char_received - '0'; // Convert ASCII character to its
	mov	a,_get_buffer_size_sloc1_1_0
	mov	r7,a
	add	a,#0xd0
	mov	dptr,#_get_buffer_size_char_received_131072_21
	...
	sjmp	00114$
00109$:
;	buffer.c:61: printf("Invalid input character\r\n"); // Inform the user about invalid input
	push	ar3
	push	ar2
	push	ar1
	push	ar0
	...
	pop	ar3
00114$:
;	buffer.c:65: buffer_size = (buffer_size * 10) + char_received;
	mov	dptr,#_get_buffer_size_buffer_size_65536_20
	movx	a,@dptr
	mov	r6,a
	inc	dptr
...
;	buffer.c:68: printf("%d\r\n", buffer_size); // Display the validated buffer size to the user
	...
	add	a,#0xfb
	mov	sp,a
;	buffer.c:69: return buffer_size; // Return the validated buffer size
